...
<%--	静态包含含有base、css、js文件--%>
	...
	<script type="text/javascript">
		...
		$(function (){
			...
			$("#code_img").click(function (){
				//加上?d=每次都不同的值(时间戳) 可解决不同浏览器只变一次问题
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			//给用户名框绑定失去焦点事件判断该用户名是否存在
			$("#username").blur(function (){
				//1.获取用户名
				var username = this.value;
				//2.使用Ajax请求
				$.getJSON("http://localhost:8080/10_book/userServlet","action=ajaxExistsUsername&username="+username,function (data){
...
<%--								<%=request.getAttribute("email")==null?"" : request.getAttribute("email")%>--%>
						...
						<input class="itxt" type="text" name="code" style="width: 100px;" id="code"
							   value="abcde"/>
						<img id="code_img" alt="" src="kaptcha.jpg" style="float: right; margin-right: 40px;width: 100px;height: 36px">
						<br />
						<br />
